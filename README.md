...
3. Extract the Image and flash to an SD card using a program like balana etcher.
...
28. Populate driver database:
	```
	depmod -a
29. Enable usb gadget services for usb eithernet and serial as well as a driver loading script:
	```
...
- Skip to step 36b if you don't want to re-partition the card or make a fresh one. 
33. Unmount the SD card, re-insert  if necessary. Identify the correct device with "lsblk" and set $SDCDEVICE. Then partition the SD card for a fresh install. 
	```
...
1. Create the image file:
	```
	SIZE_KB="$(($(du --apparent-size -sk picocalc-install | cut -f 1) + \
	        $(find picocalc-install | wc -l) * 4))"&& # Calculate size of Rootfs \
...
4. Now you can flash to an SD card with a tool like balana etcher.